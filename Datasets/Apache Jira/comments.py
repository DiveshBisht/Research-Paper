...
project_name = 'TOMCAT' # 'LUCENE' 'RAT'
data = [['ISSUE_ID','USER','DATE','COMMENT']]
#for tomcat
...
for j in list_bug_id:
    ...
    com_div = soup("div", {"class":"twixi-wrap verbose actionContainer"})

    for i in com_div:
      date = i.find_all("span",{"class":"date user-tz"})
      user = i.find_all("a",{"class":"user-hover user-avatar"})
      comment = i.find("div",{"class":"action-body flooded"})
      user_comment =  user[0].text 
      date_comment =  date[0].text 
      comment_body =  comment.text.strip()
      if 'ASF subversion and git services' not in user_comment and 'ASF GitHub Bot' not in user_comment and len(comment_body) > 2:    
        data.append([issue_id, user_comment, date_comment, comment_body ])


with open('commentseg.csv','w',newline='',encoding='utf-8') as fp:
